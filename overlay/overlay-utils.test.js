...
test("shouldIgnoreGlobalEnterEvent: ignores Enter within the post-show grace window", () => {
  const shownAt = 1000;
  assert.equal(shouldIgnoreGlobalEnterEvent({ tagName: "DIV" }, shownAt, 1200, 650), true);
  assert.equal(shouldIgnoreGlobalEnterEvent({ tagName: "DIV" }, shownAt, 1700, 650), false);
});
