...
test("shouldAllowGlobalEnter: blocks Enter on DRIFT_PERSIST even when not focused", () => {
  assert.equal(shouldAllowGlobalEnter("DRIFT_PERSIST", { tagName: "DIV" }), false);
  assert.equal(shouldAllowGlobalEnter("drift_persist", null), false);
});

test("shouldAllowGlobalEnter: allows Enter on other events when safe", () => {
  assert.equal(shouldAllowGlobalEnter("DRIFT_START", { tagName: "DIV" }), true);
  assert.equal(shouldAllowGlobalEnter("", { tagName: "DIV" }), true);
  assert.equal(shouldAllowGlobalEnter("DRIFT_START", { tagName: "INPUT" }), false);
});
