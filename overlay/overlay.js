...
function updatePrimaryActionForEventType() {
  const driftPersist = isDriftPersist();

  // Pattern-break: when drift persists, nudge toward a concrete recovery action.
  backBtn.classList.toggle("primary", !driftPersist);
  recoverBtn.classList.toggle("primary", driftPersist);

  if (enterHint) {
    enterHint.textContent = driftPersist ? "Enter: Recover schedule" : "Enter: Back on track";
  }
}
...
function showOverlay(payload) {
  ...
  updatePrimaryActionForEventType();
...
window.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Enter") {
    ...
    if (!ignoreEnter) {
      // Pattern-break: when drift persists, make Enter trigger a concrete recovery step.
      if (isDriftPersist()) {
