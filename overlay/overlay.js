...
window.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Enter") {
    // Prefer the actually-focused element. For bubbling keydown events, event.target can be <body>
    // even when a control (e.g. input/button) is focused.
    const focusTarget = document.activeElement || event.target;
    const ignoreEnter = window.overlayUtils?.shouldIgnoreGlobalEnter?.(focusTarget);
    if (!ignoreEnter) {
