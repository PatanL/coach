...
test("getInitialFocusId: prefers typing target in align mode", () => {
  assert.equal(getInitialFocusId({ eventType: "DRIFT_PERSIST", mode: "align" }), "alignText");
});

test("getInitialFocusId: DRIFT_PERSIST focuses recover", () => {
  assert.equal(getInitialFocusId({ eventType: "DRIFT_PERSIST", mode: "" }), "recoverBtn");
});

test("getInitialFocusId: default focuses back on track", () => {
  assert.equal(getInitialFocusId({ eventType: "DRIFT_START", mode: "" }), "backBtn");
});
